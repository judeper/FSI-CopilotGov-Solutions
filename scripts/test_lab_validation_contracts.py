...
class LabValidationTests(unittest.TestCase):
    ...
    def test_solution_11_risk_tiered_rollout_contract_is_required(self) -> None:
        """Solution 11's lab contract is a required, repository-tracked, read-only contract."""
        contract_path = (
            ROOT
            / "solutions"
            / "11-risk-tiered-rollout"
            / "lab"
            / "11-risk-tiered-rollout.lab.json"
        )
        self.assertTrue(
        ...
        self.assertEqual(document["solution"]["id"], "11-risk-tiered-rollout")
        self.assertEqual(
            document["mutations"],
            [],
            msg="first Solution 11 lab cycle must be read-only (mutations: [])",
        )
