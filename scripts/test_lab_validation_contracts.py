...
class LabValidationTests(unittest.TestCase):
    ...
    def test_solution_09_feature_management_controller_contract_is_required(self) -> None:
        """Solution 09's lab contract is a required, repository-tracked, read-only contract."""
        contract_path = (
            ROOT
            / "solutions"
            / "09-feature-management-controller"
            / "lab"
            / "09-feature-management-controller.lab.json"
        )
        self.assertTrue(
        ...
        self.assertEqual(document["solution"]["id"], "09-feature-management-controller")
        self.assertEqual(
            document["mutations"],
            [],
            msg="first Solution 09 lab cycle must be read-only (mutations: [])",
        )
