...
| 10-connector-plugin-governance | internal, confidential | commercial | 365 | 365 | Generated outputs contain connector and plugin inventory metadata only; live connector telemetry and manually captured user-access evidence are not collected by the scaffold. Any separately collected validation evidence must be minimized and sanitized. |
