...
Describe 'Solution 17 — SharePoint Permissions Drift Detection' {
    ...
    Describe 'Configuration content' {
        ...
        It 'has consistent controls across all tier configs' {
            $defaultConfig = Get-Content -Path (Join-Path $script:solutionRoot 'config\default-config.json') -Raw | ConvertFrom-Json
            $tiers = @('baseline', 'recommended', 'regulated')
            foreach ($tier in $tiers) {
                $tierConfig = Get-Content -Path (Join-Path $script:solutionRoot "config\$tier.json") -Raw | ConvertFrom-Json
    ...
    Describe 'Dependency declaration' {
        ...
        It 'references upstream solution in documentation' {
            ...
            $script:readme | Should -Match '02-oversharing-risk-assessment'
        }
    }
