...
Review [docs/prerequisites.md](docs/prerequisites.md) for the required admin roles, Azure and Microsoft 365 prerequisites, model inventory sources, authenticated Azure CLI (`az`) setup for lab identity verification, required `EXPECTED_AZURE_SUBSCRIPTION_ID` / `EXPECTED_TENANT_ID` environment variables, PowerShell modules, and content safety prerequisites before deploying this solution.
