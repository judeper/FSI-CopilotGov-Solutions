...
|-------|--------------|-------------|
| UAL is not capturing Copilot events | Audit is not enabled, the tenant does not have the required audit level, or recent activity has not propagated yet | Confirm Unified Audit Log is enabled, verify the tenant has Standard or Advanced audit coverage as required, wait up to 24 hours, and re-run the audit completeness check |
...
| eDiscovery case creation fails | The operator lacks the required permissions or the case role group is incomplete | Confirm eDiscovery Manager access, case permissions, and hold management roles before retrying |
