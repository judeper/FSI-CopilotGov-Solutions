...
| `GET https://graph.microsoft.com/v1.0/copilot/reports/getMicrosoft365CopilotUsageUserDetail(period='D30')` | Retrieve most recent activity data for enabled Microsoft 365 Copilot users for inactivity and utilization analysis. | Response is a report stream (CSV by default; append `?$format=application/json` for JSON). Only Microsoft 365 Copilot-licensed users are returned; tracking per-user Copilot prompt counts across tenants is not supported. Requires `Reports.Read.All`. |
| `GET https://graph.microsoft.com/v1.0/copilot/reports/getMicrosoft365CopilotUserCountSummary(period='D30')` | Retrieve the aggregated number of active and enabled Microsoft 365 Copilot users for management trend reporting. | Useful for Power BI trend cards and consistency checks. Supported periods: `D7`, `D30`, `D90`, `D180`, `ALL`. |

> **Note:** The Microsoft 365 Copilot usage report functions (`getMicrosoft365CopilotUsageUserDetail` and `getMicrosoft365CopilotUserCountSummary`) are generally available under the Microsoft Graph `v1.0` endpoint on the `/copilot/reports/` path (verified on Microsoft Learn, 2026-07-14). A separate, older `reportRoot` surface on the `/beta/reports/` path remains preview-only; prefer the `v1.0/copilot/reports/` functions for production use. Both functions require the `Reports.Read.All` permission plus a Microsoft Entra role such as Reports Reader, and they return only users who hold a Microsoft 365 Copilot license. When the tenant's usage-report anonymization setting is enabled, user principal names and display names are returned as concealed (hashed) values, so evidence derived from these reports must be handled accordingly.
...
Organizations may choose between individual pay-as-you-go billing (metered per Copilot Credit) or prepaid Copilot Studio capacity packs (25,000 Copilot Credits per month per pack). Copilot Credits are the unit of consumption for Copilot Chat and agents, and the number of credits an interaction consumes varies by interaction type — for example, classic answers, generative answers, agent actions, tenant-graph grounding, and AI tools each consume different amounts. Employee-facing agent usage is generally included at no additional charge for users already licensed with Microsoft 365 Copilot, so consumption-based charges primarily apply to unlicensed Copilot Chat and custom or SharePoint agent scenarios. Per-credit and per-pack pricing changes over time; consult Microsoft's current [pay-as-you-go meters](https://learn.microsoft.com/microsoft-365/copilot/pay-as-you-go/meters) and [Copilot Studio billing rates](https://learn.microsoft.com/microsoft-copilot-studio/requirements-messages-management) documentation for authoritative rates rather than relying on figures reproduced here. This solution documents the pattern for tracking consumption across both models, including credit volume by user and department, capacity pack utilization percentage, and overage forecasting. These patterns support compliance with SOX 404 cost allocation expectations by providing traceable spend data at the business-unit level.
