...
Beginning January 2025, Microsoft introduced consumption-based pricing for Copilot features, including pay-as-you-go Copilot Credits (metered per credit) and prepaid Copilot Studio capacity packs (25,000 Copilot Credits per month per pack). Per-credit and per-pack pricing changes over time and is published in Microsoft's pay-as-you-go meters and Copilot Studio billing-rates documentation; this solution does not reproduce specific rates. It documents governance patterns for billing policy assignment by security group, distribution group, or entire tenant, Azure Cost Management budget alert thresholds, high-usage user identification, and departmental chargeback models that support compliance with SOX 404 cost allocation and OCC 2011-12 technology expense oversight requirements. Budget alerts notify reviewers as spend approaches configured thresholds; they do not enforce a hard budget limit or stop usage.
...
See [docs/architecture.md](./docs/architecture.md) for the full data flow, endpoint inventory, and Power BI dataset design.

## Quick Start

1. Review [prerequisites](./docs/prerequisites.md) and confirm Copilot, Viva Insights, Power BI, and Graph permission readiness.
...
- Copilot Dashboard and Viva impact data availability confirmed for the tenant license mix and selected tier.
- Microsoft Graph application permissions: `Reports.Read.All` for usage reports, `LicenseAssignment.Read.All` for license inventory via `subscribedSkus`, and `User.Read.All` for user inventory. Use `Directory.Read.All` only as a higher-privileged alternative when already approved.
...
| 4.6 | Microsoft Viva Insights -- Copilot Impact Measurement | [Portal Walkthrough](../../docs/playbooks/control-implementations/4.6/portal-walkthrough.md) / [PowerShell Setup](../../docs/playbooks/control-implementations/4.6/powershell-setup.md) / [Verification and Testing](../../docs/playbooks/control-implementations/4.6/verification-testing.md) / [Troubleshooting](../../docs/playbooks/control-implementations/4.6/troubleshooting.md) |
| 4.8 | Cost Allocation and License Optimization | [Portal Walkthrough](../../docs/playbooks/control-implementations/4.8/portal-walkthrough.md) / [PowerShell Setup](../../docs/playbooks/control-implementations/4.8/powershell-setup.md) / [Verification and Testing](../../docs/playbooks/control-implementations/4.8/verification-testing.md) / [Troubleshooting](../../docs/playbooks/control-implementations/4.8/troubleshooting.md) |
...
- Dollar-valued ROI and recovery fields are illustrative customer-provided planning assumptions from configuration, not Microsoft prices or formal accounting values; replace them with an approved tenant cost basis before use.
