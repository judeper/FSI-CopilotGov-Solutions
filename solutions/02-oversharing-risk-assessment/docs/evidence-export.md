...
For regulated deployments, this artifact is generated by default when `requireOwnerAttestation` is true and helps demonstrate that business ownership and remediation closure were documented.
...
| `remediation-queue` | 1.2, 1.3, 1.4, 2.12 | Shows governance response and prioritization for discoverability scope, permissions, and external sharing |
...
| 1.3 | monitor-only | The solution records Restricted SharePoint Search as legacy transition guidance only (retiring, with new enablement blocked starting 2026-07-31) and tracks Restricted Content Discovery planning without modifying tenant state |
